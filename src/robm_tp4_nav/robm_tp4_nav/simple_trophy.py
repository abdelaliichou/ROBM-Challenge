...
class LineFollowerGripper(Node):
    def __init__(self):
        ...
        # value to ignore initial green line
    ...
    def control_loop(self):
        ...
        # ---- STARTUP Go direct until RED ----
        if self.state == 'ALIGN_FORWARD':
            if color == 'RED':
                self.get_logger().info("🔴 RED detected → Starting forced alignment turn")
                self.state = 'ALIGN_TURN'
                cmd.twist.linear.x = 0.0
                cmd.twist.angular.z = -self.v_rotate  # RIGHT turn

            else:
                # Ignore GREEN and WHITE during startup
                self.get_logger().info("🔄 Aligning → FORWARD")
                cmd.twist.linear.x = self.v_forward
                cmd.twist.angular.z = 0.0

        # ---- ALIGNMENT MODE, turning right till we get GREEN ----
        elif self.state == 'ALIGN_TURN':
            if color == 'GREEN':
                    self.get_logger().info("🟩 GREEN detected → Alignment complete")
                    self.state = 'FOLLOWING'
                    cmd.twist.linear.x = 0.0
                    cmd.twist.angular.z = 0.0
            else:
                # Force turn until GREEN appears
                self.get_logger().info("↪️ Turning until GREEN")
                cmd.twist.linear.x = 0.0
                cmd.twist.angular.z = -self.v_rotate

        # ---- FOLLOWING MODE, starting the green, red & blue combination ----
        ...
        # 1. Catching Logic (Only if not already holding it)
        if not self.grip_closed:
            # Check if object is close (10cm)
            if self.tof < 0.10:
                self.grip_counter += 1
                # Wait for 4 stable ticks to confirm it's not noise
                if self.grip_counter >= self.grip_delay_ticks:
                    self.get_logger().info("🏆 TROPHY CONFIRMED → CLOSING")
                ...
                # Reset counter if noise
                self.grip_counter = 0
                grip_msg.position = self.grip_open_pos

        # 2. Holding Logic (Once closed, STAY closed)
        else:
            grip_msg.position = self.grip_close_pos
        
        # 3. Releasing Logic (Only at BLUE line)
        if color == 'BLUE' and self.grip_closed:
             self.get_logger().info("🟦 BLUE ZONE → RELEASING TROPHY")
             self.grip_closed = False
             grip_msg.position = self.grip_open_pos
             # Stop robot at blue
             cmd.twist.linear.x = 0.0
